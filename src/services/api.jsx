...
// ——— POWER GENERATION SERIES (24h, 7d, 30d) ———
export const fetchPowerSeries = async (range = "24H") => {
  ...
  if (range === "24H") {
    ...
    for (let h = 0; h <= 23; h++) {
      ...
      const actual =
        h <= currentHour ? Math.max(0, base + (Math.random() - 0.5) * 5) : null;
    ...
    days.forEach((d, i) => {
      const baseDaily = 285 + Math.sin(i / 2) * 50;
      const variance = (Math.random() - 0.5) * 40;
      data.push({
        hour: d,
        actual: parseFloat((baseDaily + variance).toFixed(2)),
        predicted: parseFloat((baseDaily + 5).toFixed(2)),
      });
    });
  } else if (range === "30D") {
    for (let d = 1; d <= 30; d++) {
      const baseDaily = 280 + Math.sin(d / 8) * 60;
      const variance = (Math.random() - 0.5) * 50;
      data.push({
        hour: `${d}`,
        actual: parseFloat((baseDaily + variance).toFixed(2)),
        predicted: parseFloat((baseDaily + 8).toFixed(2)),
      });
    }
  }
  return data;
...
export const fetchEnergySeries = async () => {
  ...
  return months.map((month, i) => ({
    ...
    revenue: Math.round(
      110000 + Math.sin(i / 2) * 28000 + Math.random() * 8000,
    ),
  }));
};
...
export const fetchEstimation = async () => {
  ...
  const totalEst = slots
    .reduce((s, x) => s + parseFloat(x.energy), 0)
    .toFixed(1);
  const revEst = slots
    .reduce((s, x) => s + parseFloat(x.revenue.replace("₹", "")), 0)
    .toFixed(0);
  return {
    slots,
    summary: {
