...
export default function PanelGrid() {
  ...
  useEffect(() => {
    const loadPanels = async () => {
      try {
        setError(null);
        const d = await fetchPanelStatus();
        setPanels(d);
      } catch (err) {
        setError("Failed to load panel status. Please try again.");
          ...
          {loading ? (
            <div className="grid grid-cols-6 gap-2">
              {Array.from({ length: 24 }).map((_, i) => (
                <div
                  key={i}
                  className="h-14 bg-slate-200 dark:bg-void-600 rounded-lg animate-pulse"
                />
              ))}
            </div>
          ) : (
                    ...
                    {panels
                      ...
                      .map((panel) => (
                        <button
                          ...
                          onClick={() =>
                            setSelected(
                              selected?.id === panel.id ? null : panel,
                            )
                          }
                          className={clsx(
                            ...
                            selected?.id === panel.id &&
                              "ring-1 ring-solar-500 border-solar-500/60",
                          )}
                        >
                          <div className="text-[8px] font-mono text-slate-600 dark:text-void-200">
                            {panel.id}
                          </div>
                          <div
                            className={clsx(
                              ...
                              panel.status === "active"
                                ? "bg-energy-green animate-pulse-slow"
                                : "bg-energy-rose",
                            )}
                          />
                          <div className="text-[8px] font-mono text-slate-900 dark:text-void-100">
                            {panel.efficiency.toFixed(0)}%
                          </div>
                        </button>
                      ))}
                  </div>
            ...
            <div className="bg-slate-200/70 dark:bg-void-600/70 border border-solar-500/20 rounded-xl p-4 animate-slide-up">
              <div className="flex items-center justify-between mb-3">
                ...
                <button
                  ...
                  className="text-slate-700 dark:text-void-200 hover:text-slate-900 dark:hover:text-white text-lg leading-none"
                >
                  ×
                </button>
              </div>
              <div className="grid grid-cols-4 gap-3 text-xs">
                {[
                  {
                    label: "Power",
                    val: `${selected.power} W`,
                    color: "text-solar-400",
                  },
                  {
                    label: "Efficiency",
                    val: `${selected.efficiency}%`,
                  ...
                  {
                    label: "Status",
                    val: selected.status.toUpperCase(),
                    color:
                  ...
                  <div
                    key={item.label}
                    className="bg-slate-200 dark:bg-void-700 rounded-lg p-2 text-center"
                  >
                    <div className="text-[9px] font-mono text-slate-600 dark:text-void-200 mb-1">
                      {item.label}
                    </div>
