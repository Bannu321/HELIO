    ...
    <div className="card p-5 flex flex-col gap-4">
      ...
      {data?.summary && (
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-slate-200 dark:bg-void-600/60 rounded-xl p-3 border border-solar-500/10">
            <div className="text-[9px] font-mono text-slate-600 dark:text-void-200 tracking-widest mb-1">
              DAY TOTAL EST.
            </div>
            <div className="font-display text-lg font-bold text-solar-600 dark:text-solar-400">
              {data.summary.dayTotalEstimate}
            </div>
          </div>
          <div className="bg-slate-200 dark:bg-void-600/60 rounded-xl p-3 border border-energy-green/10">
            <div className="text-[9px] font-mono text-slate-600 dark:text-void-200 tracking-widest mb-1">
              REVENUE EST.
            </div>
          ...
          <div className="bg-slate-200 dark:bg-void-600/60 rounded-xl p-3">
            <div className="text-[9px] font-mono text-slate-600 dark:text-void-200 tracking-widest mb-1">
              PEAK WINDOW
            </div>
      ...
      <div className="space-y-1.5">
        <div className="text-[10px] font-mono text-slate-600 dark:text-void-200 tracking-widest">
          HOURLY FORECAST
        </div>
        {loading
          ? Array.from({ length: 5 }).map((_, i) => (
              <div
                key={i}
                className="h-8 bg-slate-300 dark:bg-void-600 rounded animate-pulse"
              />
            ))
          : data?.slots.map((slot, i) => (
              <div
                key={i}
                className="flex items-center gap-3 bg-slate-200 dark:bg-void-600/40 hover:bg-slate-300 dark:hover:bg-void-600/70 rounded-lg px-3 py-2.5 transition-colors group"
              >
                <div className="font-mono text-[10px] text-slate-600 dark:text-void-200 w-28 flex-shrink-0">
                  {slot.slot}
                </div>
                ...
                <div className="text-[9px] font-mono text-slate-600 dark:text-void-300 w-12 text-right">
                  {slot.confidence}%
                </div>
              </div>
            ))}
