...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-slate-200 dark:bg-void-700 border border-slate-300 dark:border-void-400 rounded-xl p-3 shadow-xl text-xs font-mono">
      <div className="text-slate-700 dark:text-void-200 mb-2 font-bold">
        {label}
      </div>
      {payload.map((p) => (
        <div key={p.name} className="flex items-center gap-2 mb-1">
          ...
          />
          <span className="text-slate-700 dark:text-void-100">{p.name}:</span>
    ...
    <div className="card p-5 flex flex-col gap-4">
      <div>
        ...
        <div className="text-xs text-slate-600 dark:text-void-200 mt-0.5">
          Monthly overview · kWh sold vs ₹ earned
        </div>
      </div>
        ...
        <ResponsiveContainer width="100%" height={220}>
          ...
          >
            ...
            <Line
              ...
              stroke="#00E5A0"
              strokeWidth={2}
              dot={{ r: 3, fill: "#00E5A0", stroke: "#0F1520", strokeWidth: 2 }}
            />
          </ComposedChart>
        </ResponsiveContainer>
      )}
