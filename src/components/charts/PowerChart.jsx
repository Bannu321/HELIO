    ...
    <div className="bg-void-700 border border-solar-500/20 rounded-xl p-3 shadow-xl text-xs font-mono">
      ...
      {payload.map((p) => (
        <div key={p.name} className="flex items-center gap-2 mb-1">
          ...
          <span className="text-white font-bold">{p.value != null ? `${p.value} kW` : '—'}</span>
        </div>
      ))}
    </div>
  );
...
export default function PowerChart() {
  ...
  useEffect(() => {
    setLoading(true);
    fetchPowerSeries(tab).then(d => { setData(d); setLoading(false); });
  }, [tab]);
  ...
  return (
    <div className="card p-5 flex flex-col gap-4">
      <div className="flex items-start justify-between">
        <div>
          <div className="font-display text-sm font-bold text-white tracking-wide">Power Generation</div>
          <div className="text-xs text-void-200 mt-0.5">Real-time output · 1-hour intervals</div>
        </div>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={data} margin={{ top: 4, right: 4, left: -10, bottom: 0 }}>
            ...
            <Area
              ...
              dot={false}
              connectNulls
              activeDot={{ r: 5, fill: '#F59E0B', stroke: '#0F1520', strokeWidth: 2 }}
            />
          </AreaChart>
        </ResponsiveContainer>
      )}
