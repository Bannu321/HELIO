...
// import { Dna, Sun } from 'lucide-react';
...
const navGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { icon: "🌦", label: "Weather AI", path: "/weather" },
      { icon: "🔮", label: "Estimation", path: "/estimation" },
      { icon: "🔋", label: "Energy Flow", path: "/flow" },
      { icon: "🔋", label: "Battery Management", path: "/battery" },
    ],
  },
  {
    label: "Config",
  ...
  return (
    <aside className="w-60 flex-shrink-0 border-r border-slate-300 dark:border-void-500 bg-white dark:bg-void-800 backdrop-blur-sm flex flex-col transition-colors duration-300 overflow-y-auto">
      ...
      <div className="px-5 py-6 border-b border-slate-200 dark:border-void-600">
        <div className="flex items-center gap-3">
          <div className="relative">
            <div className="w-9 h-9 rounded-full bg-solar-500 animate-glow shadow-solar flex items-center justify-center">
          ...
          <div key={group.label} className="mb-6">
            ...
            {group.items.map((item) => {
              ...
              return (
                ...
                >
                  {/* ✨ FIX IS HERE ✨ */}
                  <span className="w-5 text-center text-base flex-shrink-0 flex items-center justify-center">
        ...
        <div className="card-glow p-4 rounded-xl">
          ...
          <div className="space-y-2.5">
            ...
            <div>
              ...
              <div className="progress-track h-1.5 bg-slate-200 dark:bg-void-600">
                <div
                  className="progress-blue"
                  style={{ width: `${overview?.batteryLevel ?? 0}%` }}
                />
          ...
          <div className="mt-4 pt-3 border-t border-slate-200 dark:border-void-600 flex items-center gap-1.5">
            <div className="w-1.5 h-1.5 rounded-full bg-energy-green animate-blink" />
            <span className="text-[10px] text-slate-600 dark:text-void-200 font-mono">
              {overview?.panelsActive ?? "—"} panels active ·{" "}
              {overview?.panelsFault ?? 0} faults
