    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-fade-in">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-void-300 mt-1 font-mono">Energy export logs and net metering financials</p>
        </div>
      ...
      <div className="bg-void-800 border border-void-700 rounded-2xl p-6 shadow-card">
        <h2 className="font-display font-bold text-white mb-6">Settlement Ledger</h2>
        <RevenueTable />
      </div>
    </div>
  );
...
function FinancialCard({ title, value, icon: Icon, color, trend }) {
  return (
    <div className="bg-void-800/50 border border-void-700 rounded-2xl p-6 shadow-card flex flex-col">
      <div className="flex justify-between items-start mb-4">
        <h3 className="text-void-300 text-sm font-medium">{title}</h3>
        <div className={`p-2 rounded-lg bg-void-900 border border-void-700 ${color}`}>
          <Icon className="w-5 h-5" />
        </div>
      </div>
      <div className="mt-auto">
        <div className="text-3xl font-display font-bold text-white mb-1">{value}</div>
        <div className="text-xs font-mono text-void-400">{trend}</div>
      </div>
    </div>
  );
}